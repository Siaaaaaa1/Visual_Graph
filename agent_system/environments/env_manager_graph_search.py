...
2. **节点属性映射**：
   - **颜色 (语义)**：代表与中心节点的原生文本特征相似度（暖色/深红代表高度相似，冷色/深蓝代表异质或领域偏移，白色为中性）。
   - **形状 (拓扑重要性)**：
     - **■ (中心节点)**：你的预测目标。
...
### 【第一轮思考逻辑建议】
<think>
[初始视觉与先验分析]：结合中心节点的标题、摘要（如果有）及初始预测。观察雷达图：1-hop 内圈有哪些高相似度（红色）节点？图中是否存在关键的 Hub（▲、▼）？系统提示的 ★ (宏观学科锚点) 与中心节点的相对位置如何？
...
<think>
[新证据融合与自我反思]：上一轮我查阅了哪些节点？结合它们返回的度数、标题和摘要，它们是支持了我的主要假设，还是推翻了之前的猜测？我之前的逻辑是否存在漏洞？
...
[当前轮次决策]：现在的信息是否已经收敛到单一明确的类别？如果是，我将调用 `final(Category)`。如果仍有核心分歧，我还需要查阅哪几个 ID（注意批次惩罚成本）？
</think>
<action>...</action>
"""
