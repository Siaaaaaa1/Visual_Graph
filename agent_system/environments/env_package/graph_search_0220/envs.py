...
class GraphSearchEnv:
    ...
    def step(self, raw_input: str):
        ...
        if action_match:
            ...
            # 尝试提取 Think (如果模型使用了标签)
            t_match = re.search(r"<think>(.*?)</think>", raw_input, re.DOTALL | re.IGNORECASE)
            if t_match: 
                think_content = t_match.group(1).strip()
            elif s_match:
                # 兼容 Thinking 模型：如果没标签，取 Summary 之前的内容作为 Think
                parts = raw_input.split("<summary>")
                if len(parts) > 0:
                    think_content = parts[0].strip()
        else:
            # 如果完全没有标签，尝试直接作为动作 (Legacy Support)
            current_action = raw_input.strip()
        
        if not current_action:
            err_obs = "Error: Invalid action format. Please strictly follow: <action>...</action>"
